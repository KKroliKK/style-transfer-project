...
async def admin_mod(message: types.Message):
    if message.from_user.id == ID_ADMIN:
        await bot.send_message(
            message.from_user.id, "Admin mode is enabled!", reply_markup=menu
        )
...
async def admin_mod(message: types.Message):
    if message.from_user.id == ID_ADMIN:
        await bot.send_message(
            ...
            reply_markup=keyboards.client.menu,
        )
        ...
        for record in read:
            await bot.send_photo(
                ...
                reply_markup=InlineKeyboardMarkup().add(
                    InlineKeyboardButton(f"Delete", callback_data=f"del {record[0]}")
                ),
            )
...
async def del_callback_run(callback_query: types.CallbackQuery):
    ...
    await callback_query.answer(
        text=f'{callback_query.data.replace("del ", "")} deleted.', show_alert=False
    )
    await bot.answer_callback_query(
        callback_query.id, text=f'{callback_query.data.replace("del ", "")} deleted.'
    )
