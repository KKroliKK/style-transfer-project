    ...
    def __call__(self, sample):
        return {
            ...
            "motion_boundaries": torch.from_numpy(
                np.array(sample["motion_boundaries"]).astype(np.bool)
            ),
            "index": sample["index"],
        }
...
class Resize:
    ...
    def resize_optical_flow(self, optical_flow):
        ...
        optical_flow_resized = cv2.resize(
            optical_flow, (self.new_width, self.new_height)
        )
    ...
    def __call__(self, sample):
        return {
            ...
            "reverse_optical_flow": self.resize_optical_flow(
    ...
    def __call__(self, sample):
        if random.random() < self.p:
            return {
                ...
                "reverse_optical_flow": self.flip_optical_flow(
                    sample["reverse_optical_flow"]
                ),
