...
class MonkaaDataset(SceneFlowDataset):
    def iterate_entries(self, root_dir, use_both_sides):
        for dirpath, dirnames, filenames in os.walk(
            os.path.join(root_dir, "frames_finalpass")
        ):
            if len(filenames) == 0:
                continue
            ...
            for i in range(1, len(filenames)):
                yield SceneFlowEntry(
                    ...
                    self.forward_optical_flow_path(
                        root_dir, scene, side, self.frame_number(filenames[i - 1])
                    ),
                    self.reverse_optical_flow_path(
                        root_dir, scene, side, self.frame_number(filenames[i])
                    ),
                    self.motion_boundaries_path(
                        root_dir, scene, side, self.frame_number(filenames[i])
                    ),
                )
    ...
    def motion_boundaries_path(self, root, scene, side, frame_number):
        return os.path.join(
            root, "motion_boundaries", scene, "into_past", side, f"{frame_number}.pgm"
        )
