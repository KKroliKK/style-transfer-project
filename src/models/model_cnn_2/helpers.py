...
def im_load(path: str, im_size: int = None, crop_size: int = None, centre_crop: bool = False) -> torch.Tensor:
    ...
    """
    transformer = get_transforms(im_size=im_size,
                                 crop_size=crop_size,
...
class DataProcessor:
    def __init__(self, im_size: int = 256, crop_size: int = 240, centre_crop: bool = False):
        self.transforms = get_transforms(im_size=im_size,
                                         crop_size=crop_size,
                                         centre_crop=centre_crop)
